...
class TimerViewModel @Inject constructor(
    ...
    private fun loadExercises() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            _uiState.update { it.copy(exercises = exercises, totalSets = exercises.firstOrNull()?.sets ?: 4) }
            startSetTimer()
        }
    }
    ...
    fun onSetDone() {
        ...
        if (state.isResting) return  // ignore taps during rest
    ...
    private fun startRestTimer(seconds: Int) {
        _uiState.update { it.copy(isResting = true, restSecondsLeft = seconds) }
        restTimerJob?.cancel()
